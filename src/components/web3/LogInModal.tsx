...
const LogInModal: React.FC<ModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="absolute top-0 left-0 w-screen h-full z-10 flex justify-center modal-overlay">
      <div className="my-auto relative flex w-[351px] rounded-2xl bg-white lg:w-[860px] lg:h-[425px]">
        <div className="relative flex flex-col gap-12 p-6 lg:px-[63px] lg:pt-[82px] lg:pb-[91px] lg:w-[435px]">
          ...
          <div className="flex flex-col gap-[18px]">
            <button className="bg-black rounded-[56px] p-4 pl-[21px] flex items-center gap-2">
              <Image width={24} height={24} src="/web3/metamask.svg" alt='Metamask' />
              <span className="text-white font-semibold text-[14px] leading-[22.4px] flex-grow text-start">Connect Metamask</span>
              <Image width={24} height={24} src="/web3/arrow.svg" alt='Arrow' />
            </button>
            <button className="bg-black rounded-[56px] p-4 pl-[21px] flex items-center gap-2">
              <Image width={24} height={24} src="/web3/walletconnect.svg" alt='WalletConnect' />
              <span className="text-white font-semibold text-[14px] leading-[22.4px] flex-grow text-start">Connect WalletConnect</span>
              <Image className="" width={24} height={24} src="/web3/arrow.svg" alt='Arrow' />
            </button>
          </div>
        </div>
        <div className="hidden lg:block">
          <Image className="rounded-r-2xl" width={425} height={425} src="/web3/zuckerberg-quest.png" alt="Zuckerberg-Quest" />
        </div>
        <button className="absolute top-6 right-6 bg-black w-6 h-6 rounded-full flex items-center justify-center p-2" onClick={onClose}>
          <Image width={8} height={8} src="/web3/close-modal.svg" alt='Close' />
        </button>
      </div>
    </div>
  )
